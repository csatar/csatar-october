...
echo "Enter the container name. Leave empty to use the default value \"csatar-octobercms3\"."
...
while true; do
    ...
    if [[ $containerName =~ ^[a-zA-Z0-9\_\-]*$ ]]; then
        if ! [ -z "$containerName" ] && [ "$(docker ps -a -f name="$containerName" | grep -w "$containerName")" ]; then
            echo "The container name is already in use, please enter another name or leave empty to use the default value \"csatar-octobercms3\"."
            continue;
        fi
...
if [ -z "$containerName" ]; then
    containerName="csatar-octobercms3"
    counter=1
    ...
    if [ "$counter" -gt 1 ]; then
        echo "The container name csatar-octobercms3 is already in use. We will use ${containerName} for this installation."
    fi
fi
